/**
 * "Which documents do you want to print?" — shown before Print / PDF whenever
 ...
 * @param {{ onPrint?: (documents: number) => unknown }} [options]
 *   onPrint is told how many documents are being sent to the print dialog, just
 *   before it opens. The site uses it for the per-user printed count; the
 *   exported HTML file passes nothing.
 */
export function printWithPicker(container, options) {
  ...
  // Nothing to choose between, or no <dialog> support: just print.
  ...
  dialog.innerHTML =
    '<div class="pp-body">' +
    '<h2 class="pp-title" id="print-picker-title">Choose what to print</h2>' +
    '<p class="pp-lede">' + articles.length + ' documents are loaded. Tick the ones to include in the PDF.</p>' +
  ...
  const sync = () => {
    ...
    go.disabled = n === 0;
    go.textContent = n === 0 ? 'Select at least one' : n === boxes.length ? 'Print all (' + n + ')' : 'Print ' + n + ' of ' + boxes.length;
  };
